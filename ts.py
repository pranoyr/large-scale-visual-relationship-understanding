...
def display_ts(predictions, frame_no, fps, th=10):
	...
	for (key, db_tensor) in db_dict.items():
		...
		match_quality_matrix = box_ops.box_iou(predictions[key].type(torch.float32), db_tensor["box"].type(torch.float32))
		...
		fill = predictions[clamped_matched_idxs_in_image]
