...
class RoIHeads(torch.nn.Module):
    ...
    def assign_pred_to_rlp_proposals(self, sbj_proposals, obj_proposals, gt_boxes, gt_labels, gt_preds):
        ...
        for sbj_proposals_in_image, obj_proposals_in_image, gt_boxes_in_image, gt_labels_in_image, \
            ...
            gt_sbj_boxes_in_image = torch.unique(
                gt_boxes_in_image[:, 0, :], dim=0)
            gt_obj_boxes_in_image = torch.unique(
                gt_boxes_in_image[:, 1, :], dim=0)
            ...
            for i in range(len(sbj_boxes)):
                sbj_indices = torch.where(
                    torch.all(gt_boxes_in_image[:, 0, :] == sbj_boxes[i], dim=1))[0]
                obj_indices = torch.where(
                    torch.all(gt_boxes_in_image[:, 1, :] == obj_boxes[i], dim=1))[0]
                matched_idx = np.intersect1d(
                    sbj_indices.cpu().numpy(), obj_indices.cpu().numpy())
                if matched_idx.any():
                    labels_in_image[i] = gt_preds_in_image[matched_idx[0]]

            labels_in_image = labels_in_image.to(
                dtype=torch.int64, device=torch.device(cfg.DEVICE))
    ...
    def postprocess_detections(self,
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            keep = box_ops.batched_nms(
                boxes, scores, labels, cfg.BOX.NMS_THRESH)
        ...
        if targets:
            ...
            sbj_cls_scores, obj_cls_scores, rlp_cls_scores = \
                self.RelDN(concat_feat, sbj_feat, obj_feat, targets,
                           data_sbj["labels"], data_obj["labels"])
            ...
            for img_id in range(num_images):
                ...
                sbj_labels = labels[img_id][sbj_inds]
                obj_labels = labels[img_id][obj_inds]
                rlp_boxes = box_utils.boxes_union(sbj_boxes, obj_boxes)

                all_sbj_labels.append(sbj_labels)
            ...
            sbj_cls_scores, obj_cls_scores, rlp_cls_scores = \
                self.RelDN(concat_feat, sbj_feat, obj_feat,
                           sbj_labels=all_sbj_labels, obj_labels=all_obj_labels)
            ...
            for i, _ in enumerate(sbj_cls_scores_list):
                ...
                rel_scores, rel_indices = torch.max(
                    rlp_cls_scores_list[i], dim=1)
                # filter "unknown"
                mask = rel_indices > 0
